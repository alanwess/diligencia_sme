<?php 
  ...
  if (isset($_GET['cidade'])) $dados_query = $_GET['cidade'];
...
?>
    ...
    <header class="page-header bg-img size-lg" style="background-image: url(<?= get_template_directory_uri(); ?>/assets/img/bg-banner1.jpeg);">
      <div class="container">
        <form action="#">
          <?php if (isset($_GET['cidade']) && $dados_query != ''): ?>
            <div class="row">
              <div class="col-xs-12 col-sm-12">
                <header class="section-header">
                  <span>Um pouco mais sobre</span>
                  <?php
                    $term = get_term_by('id', $dados_query, 'cidade'); 
                    $name = $term->name;
                    $descricao = term_description($term->ID, 'empresa');
                  ?>
                  <h2>Sobre a cidade de <?= $name; ?></h2>
                </header>
                <p class="lead text-center">
                  <?php
                    ...
                    echo 'A cidade de <b>'.$name.'</b> possui <b>'.$the_query->found_posts.'</b> nova(s) vaga(s) atrelada(s) a ela nesse momento em nossa plataforma<br>';
                    
                    if($descricao != ''){
                      echo '<div class="text-center"><small>'.$descricao.'</small></div>';
                    } else {
                      echo '<div class="text-center"><small>Nenhuma informação sobre a cidade foi encontrada neste momento :(</small></div>';
                    }
                  ?>
                </p>
              </div>
          ...
          <div class="row">
            ...
            <div class="form-group col-xs-12 col-sm-12">
              <select name="cidade" class="form-control">
                <?php
                  if (isset($_GET['cidade']) && $dados_query != '') echo '<option value="'.$dados_query.'">'.$term->name.' (Selecionado)</option>';
                  else echo '<option value="">Selecione uma cidade para filtrar</option>';
            ...
            <div class="form-group col-xs-12 col-sm-12">
              <small><font size="1px">*Selecione uma cidade da sua região logo acima para visualizar e filtrar vagas*</font></small>
            </div>
          </div>
        ...
        <div class="container">
          <div class="row">
            ...
            <?php
                ...
                echo '<div class="container text-center"><h4>Ainda não há vagas para esta cidade ou não encontramos para sua filtragem :(</h4></div>';
              endif;
            ?>
            </div>
          </div>
