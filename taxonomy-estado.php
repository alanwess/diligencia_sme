<?php 
  ...
  if (isset($_GET['estado'])) $dados_query = $_GET['estado'];
...
?>
    ...
    <header class="page-header bg-img size-lg" style="background-image: url(<?= get_template_directory_uri(); ?>/assets/img/bg-banner1.jpeg);">
      <div class="container">
        <form action="#">
          <?php if (isset($_GET['estado']) && $dados_query != ''): ?>
            <div class="row">
              <div class="col-xs-12 col-sm-12">
                <header class="section-header">
                  <span>Um pouco mais sobre</span>
                  <?php
                    $term = get_term_by('id', $dados_query, 'estado'); 
                    $name = $term->name;
                    $descricao = term_description($term->ID, 'empresa');
                  ?>
                  <h2>Sobre o estado de <?= $name; ?></h2>
                </header>
                <p class="lead text-center">
                  <?php
                    ...
                    echo 'O estado de <b>'.$name.'</b> possui <b>'.$the_query->found_posts.'</b> nova(s) vaga(s) atrelada(s) a ele nesse momento em nossa plataforma<br>';
                    
                    if($descricao != ''){
                      echo '<div class="text-center"><small>'.$descricao.'</small></div>';
                    } else {
                      echo '<div class="text-center"><small>Nenhuma informação sobre o estado foi encontrada neste momento :(</small></div>';
                    }
                  ?>
                </p>
              </div>
          ...
          <div class="row">
            ...
            <div class="form-group col-xs-12 col-sm-12">
              <select name="estado" class="form-control">
                <?php
                  if (isset($_GET['estado']) && $dados_query != '') echo '<option value="'.$dados_query.'">'.$term->name.' (Selecionado)</option>';
                  else echo '<option value="">Selecione um estado para filtrar</option>';
        ...
        <div class="container">
          <div class="row">
            ...
            <?php
                ...
                echo '<div class="container text-center"><h4>Ainda não há vagas para este estado ou não encontramos para sua filtragem :(</h4></div>';
              endif;
            ?>
            </div>
          </div>
